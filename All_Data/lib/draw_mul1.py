...
color_list = [ 'steelblue', 'sandybrown', 'limegreen', 'lightcoral', 'plum',  'darkgrey', 'lightpink', 'burlywood', 'khaki', 'lightblue', 'skyblue','sandybrown']
		...
		if line.split("	 ")[-1].rstrip() == str(area):  # 3行数据
			...
			if count == (1 + int(sys.argv[2])):
				...
				for i in range(data3.shape[1]):  # lie shu
					...
					# print(data3[:,0])		# [0.   0.1  0.07 0.07 0.13 0.17 0.18 0.28]
                                        ...
                                        ax3.barh(-0.4, data3[0][i], height= 0.8, left=Sum3 - data3[0][i], color=color_list[i], alpha= 0.5,linewidth = 0)  # left	 参数来控制叠加
					plt.yticks([-0.6,0.6])
					plt.xticks([-0,1.0])
			...
			if count == (9 + int(area)):
				...
				for i in range(data2.shape[1]):  # hang shu
					...
					ax2.barh(-0.4, data2[0][i], height= 0.8, left=Sum2 - data2[0][i], color=color_list[i], alpha= 0.5,linewidth = 0)  # left	 参数来控制叠加
					plt.yticks([-0.6,0.6])
					plt.xticks([-0,1.0])
					#plt.legend(loc='upper left', fontsize= 6)
			...
			if count == (17 + int(sys.argv[2])):
				...
				for i in range(data1.shape[1]):  # hang shu
					...
					ax1.barh(-0.4, data1[0][i], height= 0.8, left=Sum1 - data1[0][i], color=color_list[i], alpha= 0.5)  # left	 参数来控制叠加
					plt.yticks([-0.6,0.6])
					plt.xticks([-0,1.0])
					#plt.legend(loc='upper left', fontsize= 6)
