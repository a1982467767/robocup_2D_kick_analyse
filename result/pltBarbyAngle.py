...
def plt_bar(dataList, path, area):
	for elt in dataList: 
		...
		plt.bar(X-0.35,Y1,width = 0.35,facecolor = 'lightskyblue',edgecolor = 'white') 
		#width:柱的宽度 
		plt.bar(X,Y2,width = 0.35,facecolor = 'yellowgreen',edgecolor = 'white') 
		...
		#给图加text 
		for x,y in zip(X,Y1): 
			plt.text(x-0.175, y+0.5, '%.2f' % y, ha='center', va= 'bottom') 
		for x,y in zip(X,Y2): 
			plt.text(x+0.175, y+0.5, '%.2f' % y, ha='center', va= 'bottom') 
		#画曲线2 
		...
		my_y_ticks = np.arange(0, 45, 2) 
		plt.xticks(my_x_ticks) 
		plt.yticks(my_y_ticks)
		title = "moreAngle: " + team[0] + "-vs-" + team[1] + " area:" + str(area)
		plt.title(title)
		plt.savefig("{}/kick_angle/moreAngle:{}area:{}.jpg".format(path,title,str(area)))
		#plt.show()
		#plt.clf()
		plt.cla()
...
if __name__ == "__main__":  #主程序入口
	...
	print(all_path)
	for area in range(8):
		filename = "/avgkickdataByAngle" + str(area) + ".txt"
		for path in all_path:
			path = root + path
			url = path + filename
			if not os.path.exists(url):  #跳过未生成的url
				continue 
			if not os.path.exists(path + "/kick_angle"):  #创建根目录
